...
public:
    Limit_quote(const std::string &bookNo, const double price, std::size_t min_qty, const double disc, const double max_q) : Bulk_quote(bookNo, price, min_qty, disc), max_qty(max_q) {}

    double total_price(std::size_t numSold) const override {
        if(numSold >= min_qty && numSold <= max_qty)
            return price * (1 - discount) * numSold;
        return price * numSold;
    }
...
int main(int argc, char* argv[]) {
    ...
    Limit_quote book3("1-99-66", 10.0, 10, 0.5, 20);
    print_total(std::cout, book1, 20);
    print_total(std::cout,  book2, 10);
    print_total(std::cout, book3, 15);
    print_total(std::cout, book3, 30);
    return 0;
}